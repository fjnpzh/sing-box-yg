...
l_link="vless://$UUID@$IP:$vless_port?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$reym&fp=chrome&pbk=$public_key&type=tcp&headerType=none#-reality"
echo "$vl_link" > jh.txt
...
if [[ -e $WORKDIR/config.json ]]; then
  ...
  mkdir -p "$HOME/bin"
  curl -Ls https://raw.githubusercontent.com/fjnpzh/sing-box-yg/main/serv01.sh > "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
